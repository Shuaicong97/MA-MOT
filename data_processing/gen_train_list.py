...
def save_image_paths(training_set, training_dir, output_file):
    ...
    for folder_name in training_set:
        folder_path = os.path.join(training_dir, folder_name, 'img1')
            ...
            for file_name in sorted(os.listdir(folder_path)):  # Ensure sorted file order
                if file_name.endswith('.jpg'):  # Only process .jpg files
                    ...
                    relative_path = os.path.relpath(os.path.join(folder_path, file_name),
                                                    start=training_dir.split("/MOT20")[0])
...
if __name__ == "__main__":
    ...
    training_dir = "/nfs/data3/shuaicong/refer-mot17/MOT17/training"
    ...
    # 读取 training.json 文件
    training_json_path = "tools/MOT17-training-videos-name.json"  # 替换为实际的 json 文件路径
    ...
    print(training_set)
    output_file_full = "datasets/data_path/refer-mot17.train"
